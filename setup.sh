...
# Edit this file to change settings, then re-run: sudo ./setup.sh
EOF
...
# Ensure .env is not world-readable (it stores secrets)
chmod 600 "$ENV_FILE"
chown "${SUDO_USER:-root}:${SUDO_USER:-root}" "$ENV_FILE"

# ---------------------------------------------------------------------------
# Determine mDNS hostname — prompt if not already set in .env
# ---------------------------------------------------------------------------
if [[ -z "${MDNS_HOSTNAME:-}" ]]; then
    ...
    read -r -p "Use '$CURRENT_HOSTNAME' as the mDNS hostname? [Y/n]: " HOSTNAME_CHOICE
    if [[ "${HOSTNAME_CHOICE,,}" == "n" ]]; then
        while true; do
            read -r -p "Enter new mDNS hostname: " MDNS_HOSTNAME
            if [[ "$MDNS_HOSTNAME" =~ ^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$ ]]; then
                break
            fi
            echo "Invalid hostname. Must be 1–63 chars, lowercase letters, digits, and hyphens only."
        done
    else
        MDNS_HOSTNAME="$CURRENT_HOSTNAME"
    fi
    printf '\nMDNS_HOSTNAME=%s\n' "$MDNS_HOSTNAME" >> "$ENV_FILE"
    echo "MDNS_HOSTNAME='$MDNS_HOSTNAME' saved to $ENV_FILE"
fi

# Validate MDNS_HOSTNAME (catches values loaded from an existing .env)
if [[ ! "$MDNS_HOSTNAME" =~ ^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$ ]]; then
...
# ---------------------------------------------------------------------------
# Require ADMIN_PIN — prompt if not already set in .env
# ---------------------------------------------------------------------------
if [[ -z "${ADMIN_PIN:-}" ]]; then
    echo
    echo "Initial admin password is required (minimum 4 characters)."
    while true; do
        read -r -s -p "Enter admin password: " ADMIN_PIN
        echo
        if [[ ${#ADMIN_PIN} -ge 4 ]]; then
            break
        fi
        echo "Password must be at least 4 characters. Please try again."
    done
    printf '\n# Initial admin password\nADMIN_PIN=\x27%s\x27\n' "${ADMIN_PIN//\x27/\x27\"\x27\"\x27}" >> "$ENV_FILE"
    echo "ADMIN_PIN saved to $ENV_FILE"
fi
...
# ---------------------------------------------------------------------------
# If the user chose a new hostname, update the system hostname to match
CURRENT_HOSTNAME="$(hostnamectl hostname 2>/dev/null || hostname)"
if [[ "$CURRENT_HOSTNAME" != "$MDNS_HOSTNAME" ]]; then
    hostnamectl set-hostname "$MDNS_HOSTNAME"
    echo "System hostname changed: $CURRENT_HOSTNAME -> $MDNS_HOSTNAME"
fi
